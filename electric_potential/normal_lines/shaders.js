...
#define MAX_CHARGES 50
#define V_SCALING_FACTOR 0.04
#define LINE_LENGTH 0.05

attribute vec3 a_position; // x,y is vector base, z is if head of vector or not
...
float compute_V(vec2 pos) {
    float V = 0.0;
    for (int i = 0; i < MAX_CHARGES; i++){
        // skip charges that are not being used
        if (u_charges[i].z == 0.0) continue;
        vec2 r = u_charges[i].xy - pos.xy;
        float Vi = -1.0 * V_SCALING_FACTOR * u_charges[i].z / length(r);
        V += Vi;
    }
    // if (V > V_MAX) V = V_MAX;
...
void main(){
    vec3 p = vec3(a_position.x, compute_V(a_position.xy), a_position.y);
    if (a_position.z == 0.0) {
        p += vec3(compute_normal(a_position.xy)) * LINE_LENGTH;
    }
    gl_Position = u_world_matrix * vec4(p, 1);
    color = vec4(1, 1, 1, 1);
}
