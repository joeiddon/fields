...
/*
 * TODO:
 * add contour lines / better color schems
 * draw lines (in 3d) of the equipotentials (contours) - try and work out how
 * write a library to deal with vectors
 * code to map mouse pointer to surface properly
 * investigate if worth while to change from gl.STATIC
 */
...
let lines = [];
let line_divisions = 40;
for (let x = 0; x <= line_divisions; x ++) {
    for (let y = 0; y <= line_divisions; y ++) {
        let point = [x / line_divisions * 2 - 1, y / line_divisions * 2 - 1];
        lines.push(...point, 0); //push tail
        lines.push(...point, 1); //push head
    }
}
let line_vector_bases_buffer = gl.createBuffer();
gl.bindBuffer(gl.ARRAY_BUFFER, line_vector_bases_buffer);
